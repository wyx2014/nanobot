...
Use this file for project-specific facts, recurring workspace conventions, and instructions the agent should follow in this workspace. Keep durable facts about the user in `USER.md` and global personality/style guidance in `SOUL.md`. `memory/MEMORY.md` is legacy compatibility only; do not put new project or session facts there.

## Scheduled Reminders

- Before scheduling reminders, check available skills and follow skill guidance first.
- Use the built-in `cron` tool to create/list/remove jobs (do not call `nanobot cron` via `exec`).
- Get USER_ID and CHANNEL from the current session metadata; do not guess or substitute identifiers from another channel.
